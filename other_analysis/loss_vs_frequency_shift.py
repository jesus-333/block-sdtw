...
config_loss = dict(
    ...
    edge_samples_ignored = 0,           # Ignore this number of samples during the computation of the reconstruction loss
    ...
    bandwidth = None,                   # Bandwidth for the pruned DTW (None for no bandwidth)
    device = device,
    # device = "cpu",
)

# Adjust shape of the sine wave to match the expected input of the loss function
...
loss_values_sdtw = []
loss_values_pruned_dtw = np.zeros((len(bandwidth_list), len(f_2_list))) if len(bandwidth_list) > 0 else None

# Compute loss for block-SDTW
config_loss['recon_loss_type'] = 4
for idx_block in range(len(block_size_list)) :
    block_size = block_size_list[idx_block]
    ...
    for i in range(len(f_2_list)) :
        # Generate the second sine wave with frequency f_2
        f_2 = f_2_list[i]
        samples_2 = 2 * np.pi * f_2 * t_samples + shift_2
        sine_2 = torch.from_numpy(amplitude_2 * np.sin(samples_2))

        # Adjust the shape of the sine wave to match the expected input of the loss function
...
# Copute loss for pruned DTW
config_loss['recon_loss_type'] = 2
for idx_bandwidth in range(len(bandwidth_list)) :
    bandwidth = bandwidth_list[idx_bandwidth]
    config_loss['bandwidth'] = bandwidth
    loss_function_pruned_sdtw = reconstruction_loss(config_loss)
    print("---------------------------------------------------------")
    print("Compute loss for pruned DTW with bandwidth : {}".format(bandwidth))
    ...
    for i in range(len(f_2_list)) :
        ...
        # loss_function = reconstruction_loss(config_loss)
        tmp_loss = float(loss_function_pruned_sdtw.compute_loss(sine_2, sine_1).cpu().numpy())

        loss_values_pruned_dtw[idx_bandwidth, i] = tmp_loss
...
# Plot the loss values for SDTW
ax.plot(f_2_list, loss_values_sdtw,
        label = "SDTW", color = 'tab:red',
        marker = 'D', markersize = plot_config['marker_size']
        )

# Plot the loss values for Pruned DTW
if loss_values_pruned_dtw is not None :
    colors_list = ['tab:purple', 'violet']
    for i in range(len(bandwidth_list)) :
        ax.plot(f_2_list, loss_values_pruned_dtw[i],
                color = colors_list[i], label = "Pruned DTW - bandwidth = {}".format(bandwidth_list[i]),
                marker = 'o', markersize = plot_config['marker_size'],
                )


# Plot the loss values for Block-SDTW
colors_list = ['tab:blue', 'blue']
for i in range(len(block_size_list)) :
    ax.plot(f_2_list, loss_values_per_block_size[i],
            color = colors_list[i], label = "Block-SDTW - block size = {}".format(block_size_list[i]),
...
if plot_config['save_fig'] :
    ...
    fig.savefig(path_save + 'f1_{}Hz_A1_{}_f2_{}Hz_A2_{}.pdf'.format(f_1, amplitude_1, f_2, amplitude_2))
    fig.savefig(path_save + 'f1_{}Hz_A1_{}_f2_{}Hz_A2_{}.eps'.format(f_1, amplitude_1, f_2, amplitude_2))
