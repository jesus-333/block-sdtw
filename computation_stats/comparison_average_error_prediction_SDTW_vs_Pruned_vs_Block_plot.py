...
if plot_config['n_samples_to_predict'] > 0 :
    path_errors_matrix_folder = f"./saved_model/block_size_{plot_config['block_size']}_predict_samples_{plot_config['n_samples_to_predict']}/0_comparison/"
else :
    path_errors_matrix_folder = f"./saved_model/block_size_{plot_config['block_size']}_predict_portion_{int(plot_config['portion_of_signals_for_input'] * 100)}/0_comparison/"

if plot_config['normalize_0_1_range'] :
    path_errors_matrix_folder += "normalized_"

# Matrix to store the average errors
matrix_to_plot_train = np.load(path_errors_matrix_folder + "average_errors_matrix_train.npy")
matrix_to_plot_test  = np.load(path_errors_matrix_folder + "average_errors_matrix_test.npy")
...
if plot_config['clip_results'] :
    ...
    for i in range(matrix_to_plot_train.shape[0]) :
        ...
        if np.sum(row_train != 0) > 0 :
            # Note that if the sum of the row is 0 it means that the dataset was skipped during training/testing  w
...
for i in range(2) :
    ...
    axs[i].set_yticks(np.arange(3), minor = False)
    axs[i].set_yticklabels(['SDTW', 'Pruned SDTW', 'Block SDTW'])
